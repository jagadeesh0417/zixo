...
export async function GET(
  ...
  try {
    ...
    const order = placeholderOrders.find((o) => o.id === id);
    if (!order) {
...
export async function PUT(
  ...
  try {
    const { id } = await params;
    const index = placeholderOrders.findIndex((o) => o.id === id);
    if (index === -1) {
      return NextResponse.json({ error: "Order not found" }, { status: 404 });
    }

    const body = await request.json();
    const updatedOrder = { ...placeholderOrders[index], ...body, id, updatedAt: new Date().toISOString() };
